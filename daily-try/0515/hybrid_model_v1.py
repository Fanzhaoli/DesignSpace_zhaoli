...
# -*- coding: utf-8 -*-
"""
混合模型 V1.0 - 结合 verify_best_model 和 V2.3 模型的优点
根据T值自动切换模型行为
"""
...
class HybridDDMModel:
    ...
    def get_model_weight(self, T):
        return 1 / (1 + np.exp(-(T - self.t_switch) / self.t_width))
    
    def compute_v_v23(self, T, P, condition):
    ...
    def compute_v_best(self, T, P, condition):
        T_0 = 100
        k_T = 0.01
        ...
        if condition == 'self':
            t_eff = np.exp(-((T - alpha1_t_center)**2) / (2 * alpha1_t_width**2))
            alpha = alpha1_base * t_eff
            v = v_0 * (1 + alpha)
        else:
            t_eff = np.exp(-((T - alpha1_t_center)**2) / (2 * alpha1_t_width**2))
            p_eff = 0.2 / (1 + np.exp(-0.05 * (P - 32)))
            alpha = alpha2_base * (1 + t_eff * p_eff)
            v = v_0 * (1 + alpha)
        
        return min(v, 2.5)
    
    def compute_v(self, T, P, condition):
        w = self.get_model_weight(T)
        v_v23 = self.compute_v_v23(T, P, condition)
        v_best = self.compute_v_best(T, P, condition)
        return w * v_v23 + (1 - w) * v_best
...
def main():
    print("="*80)
    print("混合模型 V1.0 - 结合 verify_best_model 和 V2.3")
    print("="*80)
    ...
    for design, P, T, W in designs:
        ...
        weight = model.get_model_weight(T)
    ...
    print("\n模型预测 vs 实证数据")
    print("="*80)
    print(df[['Design', 'T', 'Model_weight', 
              'RT_diff_model', 'RT_diff_emp', 'RT_error',
    ...
    print(f"\nD5 条件:")
    print(f"  预测 RT_diff: {d5_result['RT_diff_model']:.4f}")
    print(f"  目标 RT_diff: {empirical['D5']['RT_diff']}")
    print(f"  误差: {d5_result['RT_error']:.4f}")
    
    df.to_csv('hybrid_model_v1_results.csv', index=False)
    print(f"\n结果已保存到 hybrid_model_v1_results.csv")
