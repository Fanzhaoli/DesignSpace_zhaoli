...
file_emp <- list.files(
  here::here("Study4", "data", "emp_data"), 
  pattern = "EXP.*\\.csv$", 
  full.names = TRUE)
```
...
bruceR::export(
  paradata, 
  file = here::here("Study4", "data", "para_data.csv"))
```

```{r}
# 确保文件路径正确
file_path <- here::here("Study4", "data", "fast dm", "estimates_0424.txt")  
...
# 将所有需要读取的数据文件名字保存到名为files的list
file_sim <- list.files(
  here::here("Study4", "data", "sim_data", "final", "250430"), 
  pattern = "d*\\.csv$", 
  full.names = TRUE)
```
